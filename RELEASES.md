...
The scanner has had **an independent security review by an industry professional** (unnamed) of **v0.1.0**
(first pass) and **v0.3.7** (re-review). Every core change since those builds is recorded in `CHANGELOG.md`,
so a reviewer can diff the reviewed tree against any later tag — `git diff v0.3.7 vX.Y.Z`. **The reviewed
builds were v0.1.0 / v0.3.7; re-validation of the current core is pending.** Within a single version the
core is deterministic: same input, same output, byte-for-byte. The review record (findings + remediation)
is in `CHANGELOG.md` and `FOR_AUDITORS.md`.
...
| 0.4.0 | `v0.4.0` | 2026-07-16 | security — independent-review remediation, re-review pass (HS-01/02/03/04 + fetch-time-RCE closed, all in optional tiers) |
| 0.3.7 | `v0.3.7` | 2026-07-13 | security — independent-review remediation (HS findings closed with adversarial PoCs) |
...
| 0.1.0 | `v0.1.0` | 2026-07-09 | initial standalone scanner — the build put through the first independent security review by an industry professional |
